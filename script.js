...
document.getElementById('formCalculadora').onsubmit = (e) => {
    ...
    // ==========================================
    // A MÁGICA DO PDF DINÂMICO E LOGO À DIREITA
    // ==========================================
    
    // Preparação da Tag da Logo Dinâmica
    let tagLogoPdf = '';
    if (configVisualNuvem.logo) {
        // Ajuste de tamanho baseado no slider (max 80px altura)
        tagLogoPdf = `<img src="${configVisualNuvem.logo}" style="max-height: 80px; max-width: ${configVisualNuvem.tamanho}%; display: block;">`;
    }
    ...
    <div id="pdfContent" style="padding: 30px; font-family: Arial, sans-serif; position: relative; overflow: hidden; background: white; color: black; min-height: 800px;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 5px; border: none;">
            <tr>
                ...
                <td style="width: 50%; text-align: right; vertical-align: middle; border: none; padding-bottom: 10px;">
    ...
    if (incluirInsumos) {
        ...
        html += `<h3 style="color: #2c3e50; margin-top: 20px; font-size: 16px;">2. Materiais de Instalação</h3>
            <table class="pdf-table" style="font-size: 13px;">
                <tr style="background: ${configVisualNuvem.cor}; color: #fff;"><th>Insumo</th><th>Qtd.</th><th>Valor Unit.</th><th>Subtotal</th></tr>
                <tr><td>Argamassa Polimérica (Saco 20kg)</td><td>${sacos} scs</td><td>R$ ${pArg.toFixed(2)}</td><td>R$ ${(sacos*pArg).toLocaleString('pt-BR', {minimumFractionDigits:2})}</td></tr>
                <tr><td>Espuma PU (Tubo 500ml)</td><td>${tubos} un.</td><td>R$ ${pPU.toFixed(2)}</td><td>R$ ${(tubos*pPU).toLocaleString('pt-BR', {minimumFractionDigits:2})}</td></tr>
                <tr><td>Tela de Fibra (Rolo 50m)</td><td>${rolos} rls</td><td>R$ ${pTela.toFixed(2)}</td><td>R$ ${(rolos*pTela).toLocaleString('pt-BR', {minimumFractionDigits:2})}</td></tr>
            </table>`;
    ...
    html += `<div class="resumo-financeiro" style="font-size: 14px;">
    ...
    if (incluirCronograma) {
        html += `<h3 style="color: #2c3e50; margin-top: 25px; font-size: 16px;">3. Cronograma Estimado de Montagem</h3>
    ...
    if (incluirComparativo) {
        ...
        html += `<h3 style="color:#2c3e50; margin-top:25px; font-size: 16px;">${numeracao}. Análise de Viabilidade Financeira</h3>
            ...
            <div style="text-align:center; margin-top:15px; padding:10px; color:${configVisualNuvem.cor}; font-weight:bold; border-radius: 4px; border: 1px dashed ${configVisualNuvem.cor}; font-size: 14px;">Economia Estimada: R$ ${economiaReais > 0 ? economiaReais.toLocaleString('pt-BR', {minimumFractionDigits:2}) : "0,00"}</div></div>`;
    }

    html += `<p style="font-size:10px; text-align:center; margin-top:30px; color: #95a5a6;">* Documento gerado digitalmente.</p></div>`;
